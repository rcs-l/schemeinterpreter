...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        SchemeError('non-symbol: {0}'.format(bad_signature))
...
def make_let_frame(bindings, env):
    ...
    names, vals = Pair(bindings.first.first, nil), Pair(scheme_eval(bindings.first.rest.first, env), nil)
    bindings = bindings.rest
    while bindings is not nil:
        bind = bindings.first
        validate_form(bind, 2, 2)
        names.rest, vals.rest = Pair(bind.first, nil), Pair(scheme_eval(bind.rest.first, env), nil)
        bindings = bindings.rest
